const gameModule = (() => {
  ...
  playersForm.addEventListener("submit", (event) => {
    ...
    if (!playersForm.checkValidity())
      playersForm.classList.add("was-validated");
  ...
  btnPlay.addEventListener('click', () => {
    ...
    console.log('complete the players and rules to play');
  });

  const renderPlayersHtml = () => {
    if (players.length > 0) {
      let htmlPlayers = [];
      for (let i = tbodyplayers.childNodes.length - 1; i < players.length; i++) {
        htmlPlayers.push(`<tr><th scope="row">${i+1}</th><td>${players[i]}</td></tr>`);
      }
  ...
  const renderRulesHtml = () => {
    if (rules.length > 0) {
      ...
      for (let i = tbodyrules.childNodes.length - 1; i < rules.length; i++) {
        htmlRules.push(`<tr><th scope="row">${i+1}</th><td>${rules[i].points}</td><td>${rules[i].value}</td></tr>`);
      }
